...
const Timer = () => {
  ...
  useEffect(() => {
    ...
    const countdownEnd = 15 * 3600 + 22 * 60; // 12:30 in seconds
    const countupEnd = countdownEnd + 1 * 60; // 10 minutes in seconds
  ...
  return (
    <div>
      {!timerStopped && <h1>{formatTime(seconds)}</h1>}
      {!timerStopped && <p>{countingDown ? "Counting Down" : "Counting Up"}</p>}
